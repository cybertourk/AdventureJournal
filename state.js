...
export function updateDerivedState() {
    ...
    if (window.appData.activeCampaign) {
        ...
        (window.appData.activeCampaign.codex || []).forEach(c => addAlias(c.name, c.id, c.type));
        (window.appData.activeCampaign.playerCharacters || []).forEach(pc => addAlias(pc.name, pc.id, 'PC'));
        (window.appData.activeCampaign.rulesGlossary || []).forEach(r => addAlias(r.name, r.id, 'Rule'));
