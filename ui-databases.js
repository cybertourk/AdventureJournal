...
import { getUnifiedCatalog, generateId, updateDerivedState } from './state.js';
...
if (typeof window !== 'undefined') {
    ...
    window.appActions.searchDatabase = (val) => {
        DATABASE_SEARCH_QUERY = val.toLowerCase().trim();
        window.appActions.renderDatabaseResults();
    };

    window.appActions.filterDatabaseCategory = (cat) => {
        DATABASE_FILTER_CATEGORY = cat;
        window.appActions.renderDatabaseResults();
    };

    window.appActions.filterDatabaseRarity = (rarity) => {
        DATABASE_FILTER_RARITY = rarity;
        window.appActions.renderDatabaseResults();
    };

    window.appActions.openDatabaseItemDetails = async (itemName) => {
        ...
        const rColor = item.rarity === 'legendary' ? 'text-orange-600 bg-orange-50 border-orange-200' : 
                       (item.rarity === 'veryrare' || item.rarity === 'very-rare' ? 'text-purple-600 bg-purple-50 border-purple-200' : 
                       (item.rarity === 'rare' ? 'text-blue-600 bg-blue-50 border-blue-200' : 
                       (item.rarity === 'uncommon' ? 'text-emerald-600 bg-emerald-50 border-emerald-200' : 
        ...
        if (stats.ac) {
            ...
            <div class="bg-stone-50 border border-stone-200 p-2.5 rounded-sm shadow-inner grid grid-cols-2 gap-2 text-xs text-stone-700">
                ...
                <div><span class="font-bold text-stone-900 block uppercase tracking-wider text-[8px]">Strength Needed</span> ${stats.strengthReq || 0}</div>
        ...
        const parsedDesc = item.description ? window.appActions.parseSmartText(item.description) : '<p class="italic text-stone-400">No descriptive lore logged.</p>';
        ...
        container.innerHTML = `
            <div class="fixed inset-0 bg-stone-900 bg-opacity-80 flex items-center justify-center p-4 z-[18000] backdrop-blur-sm animate-in">
                <div class="bg-[#f4ebd8] rounded-sm w-full max-w-md border border-[#d4c5a9] shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh] border-t-4 border-t-amber-700">
                    <button onclick="document.getElementById('global-popup-container').innerHTML = '';" class="absolute top-4 right-4 text-stone-400 hover:text-red-900 transition"><i class="fa-solid fa-times text-xl"></i></button>
                    
                    <div class="p-5 sm:p-6 overflow-y-auto custom-scrollbar flex-grow space-y-4">
                        ${item.image ? `<div class="w-full h-44 bg-stone-900 border border-stone-300 rounded-sm overflow-hidden p-2 flex justify-center"><img src="${item.image}" class="h-full object-contain"></div>` : ''}
                        
                        <div>
                            <h3 class="text-xl font-serif font-bold text-stone-900 leading-snug">${item.name}</h3>
                            <div class="flex flex-wrap gap-1.5 mt-2 text-[8px] font-bold uppercase tracking-wider">
                                <span class="px-2 py-0.5 rounded border ${rColor}">${item.rarity || 'common'}</span>
                                <span class="bg-amber-100 text-amber-800 border border-amber-200 px-2 py-0.5 rounded shadow-sm"><i class="fa-solid fa-coins mr-1 text-amber-600"></i>${(item.price || 0).toLocaleString()} gp</span>
                                <span class="bg-stone-100 text-stone-600 border border-stone-200 px-2 py-0.5 rounded shadow-sm">${item.type}</span>
                                ${item.weight ? `<span class="bg-blue-50 text-blue-800 border border-blue-200 px-2 py-0.5 rounded shadow-sm">${item.weight} lbs</span>` : ''}
                            </div>
                        </div>
    ...
    window.appActions.openItemForgeModal = async (itemId = "", prefilledName = "") => {
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="forge-item-rarity" class="w-full p-2 border border-[#d4c5a9] rounded-sm text-sm font-bold text-stone-900 shadow-inner outline-none bg-stone-50 focus:border-amber-600">
                                ...
                                <option value="custom" ${item.rarity==='custom'?'selected':''}>Custom / Artifact</option>
                            </select>
                        </div>
                        <div>
    ...
    window.appActions.updateForgeInputStats = (type) => {
    ...
    window.appActions.saveForgedItem = async () => {
        ...
        const forgedItem = {
            id, name, type, price, rarity, isMagic, image, description, weight, stats
        };
    ...
    window.appActions.deleteForgedItem = async (itemId) => {
    ...
    window.appActions.openItemJsonImporter = () => {
        const container = document.getElementById('global-popup-container');
        if (!container) return;
    ...
    window.appActions.handleItemFileSelect = (event) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (textarea) {
                    ...
                    notify("Item file loaded successfully. Click 'Parse & Save Item' to complete.", "success");
                }
            } catch (err) {
    ...
    window.appActions.executeItemJsonImport = async () => {
        ...
        displayList.forEach(item => {
            ...
            <div onclick="window.appActions.openDatabaseItemDetails('${item.name.replace(/'/g, "\\'")}')" class="bg-white border border-[#d4c5a9] rounded-sm p-2.5 shadow-sm flex justify-between items-center gap-3 hover:border-amber-400 transition cursor-pointer">
                <div class="min-w-0 flex-grow pr-2 flex items-center gap-2.5">
                    ${itemImageHtml}
                    <div class="min-w-0">
                        <span class="font-bold text-xs text-stone-900 truncate block leading-tight" title="${item.name}">${item.name}</span>
                        <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
                            <span class="text-[8px] uppercase font-bold tracking-widest ${rColor}">${item.rarity || 'Item'}</span>
                            <span class="text-[9px] font-bold text-amber-700 bg-amber-50 px-1 py-0.2 rounded border border-amber-200"><i class="fa-solid fa-coins mr-1 text-amber-500"></i>${(item.price || 0).toLocaleString()} gp</span>
                            ${customBadge}
                        </div>
                    </div>
                </div>
    ...
    <div class="animate-in fade-in duration-300 pb-12 max-w-7xl mx-auto flex flex-col h-full">
        ...
        <div class="bg-[#fdfbf7] rounded-sm border-2 sm:border-4 border-stone-800 shadow-[0_20px_50px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col flex-grow">
            ...
            <div class="p-4 sm:p-6 flex-grow flex flex-col min-h-0 bg-[#fdfbf7]">
                ...
                <!-- Database Result Grid (Lazy Loaded & Paginated) -->
