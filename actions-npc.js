...
export const generateNpcData = (locks = {}) => {
    ...
    const birthRegion = cascadeRoll([
        `Birth Region (${subrace})`, 
        `Birth Region (Human - ${subrace})`, 
        `Birth Region (${race})`, 
        `Birth Region (Master)`
    ]);
    
    let birthplace = cascadeRoll([
        `Birthplace (${subrace})`, 
        `Birthplace (${race})`
    ]);
    ...
    const age = cascadeRoll([
        ...
        `Age (Default ${race})`, `Human Age (Default)`, `Age (Default)`
    ]);
    
    const height = cascadeRoll([
        `Height (${subrace})`, `Physical - Height (${subrace})`, `${subrace} Height`, 
        `Height (${race})`, `Physical - Height (${race})`, `${race} Height (Default)`, 
        `Height (Default ${race})`, `Human Height (Default)`, `Height (Default)`
    ]);
    
    const weight = cascadeRoll([
        `Weight (${subrace})`, `Physical - Weight (${subrace})`, `${subrace} Weight`, 
        `Weight (${race})`, `Physical - Weight (${race})`, `${race} Weight (Default)`, 
        `Weight (Default ${race})`, `Human Weight (Default)`, `Weight (Default)`
    ]);
    
    const hair = cascadeRoll([
        `Hair Color (${subrace})`, `Hair (${subrace})`, `Physical - Hair Color (${subrace})`, `${subrace} Hair Color`, 
        `Hair Color (${race})`, `Hair (${race})`, `Physical - Hair Color (${race})`, `${race} Hair Color (Default)`, 
        `Hair Color (Default ${race})`, `Human Hair Color (Default)`, `Hair (Default)`
    ]);
    
    const eyes = cascadeRoll([
        `Eye Color (${subrace})`, `Physical - Eye Color (${subrace})`, `${subrace} Eye Color`, 
        `Eye Color (${race})`, `Physical - Eye Color (${race})`, `${race} Eye Color (Default)`, 
        `Eye Color (Default ${race})`, `Human Eye Color (Default)`, `Eye Color (Default)`
    ]);
    
    const skin = cascadeRoll([
        `Skin Tone (${subrace})`, `Physical - Skin Tone (${subrace})`, `${subrace} Skin Tone`, 
        `Skin Tone (${race})`, `Physical - Skin Tone (${race})`, `${race} Skin Tone (Default)`, 
        `Skin Tone (Default ${race})`, `Human Skin Tone (Default)`, `Skin Tone (Default)`
    ]);
    
    const build = cascadeRoll([
        `Build (${subrace})`, `Build (${race})`, `Build (Default ${race})`, `Build (Human)`, `Build (Default)`
    ]);
    
    const feature = cascadeRoll([
        `Feature (${subrace})`, `Distinguishing Feature (${subrace})`, `Feature (${race})`, `Distinguishing Feature (${race})`, 
    ...
    const firstName = cascadeRoll([
        `${firstNameBase} (${subrace})`,
        `Names (${altGender}, ${subrace})`,
        `${subrace} First Name - ${altGender}`,
        `${firstNameBase} (${race})`,
        `Names (${altGender}, ${race})`,
        `${race} First Name - ${altGender}`,
        `${firstNameBase} (Default ${race})`,
        `Names (${altGender}, High Elf)`,
        `Names (${altGender}, Wood Elf)`,
        `Female Name (Default Human)` // Universal fallback if all else fails
    ]);

    const lastName = cascadeRoll([
        `Last Name (${subrace})`,
        `Names (Last, ${subrace})`,
        `Names (Tribe, ${subrace})`,
        `${subrace} Last Name`,
...
const renderNpcPreviewModal = (data) => {
    ...
    // Critically bind the global actions so the DOM buttons can fire them
    ...
    container.innerHTML = `
        <div class="fixed inset-0 bg-stone-900 bg-opacity-80 flex items-center justify-center p-4 z-[18000] backdrop-blur-sm">
            <div class="bg-[#f4ebd8] rounded-sm w-full max-w-md shadow-2xl relative flex flex-col max-h-[90vh]">
...
export const executeNpcGeneration = () => {
    // Gather locks from the DOM Dropdowns
...
// Global Exposure bindings just in case!
